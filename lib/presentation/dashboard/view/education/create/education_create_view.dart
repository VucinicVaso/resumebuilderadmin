...
import '../../../controller/education/create/education_create_controller.dart';

// ignore: must_be_immutable
class EducationCreateView extends WTWView<EducationCreateController> {

  EducationCreateView({ super.key }) {
    setController(EducationCreateController());
  }

  WTWUIComponent? createScaffold(EducationCreateController? con) {
    var header = uiFactory!.createHeader(WTWUIHeaderType.basic1)!
      ..setBackAction(
        action: () async { await con!.navigateBack(); }, 
        icon: Symbols.arrow_back,
        label: 'education_create'.tr
      );
