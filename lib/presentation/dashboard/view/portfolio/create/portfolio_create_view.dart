...
import '../../../controller/portfolio/create/portfolio_create_controller.dart';

// ignore: must_be_immutable
class PortfolioCreateView extends WTWView<PortfolioCreateController> {

  PortfolioCreateView({ super.key }) {
    setController(PortfolioCreateController());
  }

  WTWUIComponent? createScaffold(PortfolioCreateController? con) {
    var header = uiFactory!.createHeader(WTWUIHeaderType.basic1)!
      ..setBackAction(
        action: () async { await con!.navigateBack(); }, 
        icon: Symbols.arrow_back,
        label: 'portfolio_create'.tr
      );
